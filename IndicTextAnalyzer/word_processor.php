...
class wordProcessor {
	...
	// NEW FUNCTION EXCLUSIVE TO FINAL PROJECT
	// Function to add to logical chars table
	function toLogicalChars($word, $db)
	{		
		// Parse the word to logical characters
		$logicalChars = $this->getLogicalChars();
		
		// Iterate through each character of the word
		for($i = 0; $i < count($logicalChars); $i++)
		{
			// Create insertion query
			$query = "INSERT INTO `logical_chars` (`word`, `logical_char`, `position`) VALUES ('" . $word ."', '". $logicalChars[$i] ."', '". $i ."')";
